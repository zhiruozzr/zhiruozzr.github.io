...
<head>
  <meta charset="UTF-8" />
  <title>TIME TO RELAX · Pixel Maze PRO</title>
  <style>
    body {
      margin: 0;
      padding: 20px;
...
<body>
  <div class="panel">
    ...
    <div>
      ...
      <span class="tag">Bubble Bomb</span>
      <span class="tag">Timer & Steps</span>
    </div>
  ...
  <script>
    ...
    let traps = [];  // {row, col}
    const VISION_RADIUS = 3; // in tiles
      ...
      for (let r = 0; r < ROWS; r++) {
        ...
        for (let c = 0; c < COLS; c++) {
          ...
          if (ch === "P") {
            ...
            player.col = c;
            player.facingRow = 0;
            player.facingCol = 1; // default facing right
          } else if (ch === "E") {
            exitCell.row = r;
            exitCell.col = c;
          } else if (ch === "X") {
    ...
    function updateBombs() {
      bombs.forEach(b => {
        ...
        if (ch === "#") {
          // Solid wall: bomb disappears
          b.active = false;
          return;
        } else if (ch === "*") {
          // Cracked wall: explode and disappear
    ...
    // Bubble bomb simple shimmer animation
